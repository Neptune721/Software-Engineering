    ...
    <el-card style="border-color:transparent;background-color:#182d4a;margin-left:5%;margin-top:5%;width:1100px">
      ...
      <div style="color:white;font-family: 微软雅黑;margin-top: 50px">
        <div v-for="item in games">
          ...
          <span style="margin-left: 25px; margin-top: 100px;position:absolute;color:lightyellow"><span style="margin-left: 10px" v-for="ray in item.GENRE">{{ray}}</span></span>
          <span style="font-color:white;font-size:30px;margin-left: 50px; margin-top: 20px;position:absolute">{{item.COMMODITY_NAME}}</span>
          <span style="margin-left: 390px; margin-top: 0px;position:absolute;width:220px">"{{item.DESCRIPTION}}"</span>
  ...
  methods:
    {
      async gamelist() {
        ...
        console.log(name);
        var status = await axios.get(api+'/api/v1/SearchService/SearchByName?name='+ name)
        this.games = status.data
