<template>
  <div id="app-up">
    <el-card style="margin-left: 50px;margin-top: 50px;background-color:whitesmoke;border-color: transparent;width:420px">
      <span style="font-family: 微软雅黑;font-size:24px">上传头像</span>
    ...
    <el-link href="http://43.143.159.150:8081/#/" target="_blank" style="margin-left: 15px">点击返回首页</el-link>
  </div>
</template>
...
export default {
  ...
  methods: {
    async onsubmit(picName) {
      ...
      await axios.post(api+'/api/v1/BuyerInfoService/BuyerAvatar?BuyerID='+document.cookie+'&url='+picUrl)
    },
    handleUpload(e) {
      let _this = this,
        file = e.file
